        ...
        void measure(Model& model, T t)
        {
            if(saveScalarK) {   // kinetic energy of the scalar singlets
                ForLoop(i, 0, Model::Ns -1,
                                ...
                                fIO.saver.save(t, Energies::kineticS(model,FieldFunctionals::pi2S(model,i))+ Energies::gradientS(model,FieldFunctionals::grad2S(model,i)) +  Potential::potential(model) , "E_S_K_" + std::to_string(i));
                                fIO.saver.close();
                        );
            }
