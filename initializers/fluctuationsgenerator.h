...
namespace TempLat {
    ...
    public:
        ...
        auto getFluctuationsNorm(Model& model, Field<T> f, T mass2, T kCutOff) const
        {
            ...
            return   Hcut * pow((1/ 16 * k * pow(M_PI,3)),0.5)   * (model.omegaStar / model.fStar * pow(lSide / pow<2>(f.getDx()), 1.5));// Hcut  * (model.omegaStar / model.fStar * pow(lSide / pow<2>(f.getDx()), 1.5)) * pow(2 * omega, -0.5)/sqrt(2); ///here initializations
